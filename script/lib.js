(function($) {
    ...
    $.extend($.fn, {
        ...
        fildUpLoad: function(ele, callback) {

            if (!this.length) return false;

            $(this).on('change', '#' + ele, function(evt) {
                var file = $(this);
                var setting = {
                    file: file,
                    allowType: file.attr('accept') !== undefined ? file.attr('accept').split(',') : ['image/jpg', 'image/jpeg', 'image/png'],
                    filePath: file.val(),
                    pass: true,
                    maxSize: (file.data('max-size') || 20) * 1024 * 1024
                };
                var size = false;
                ...
                if (setting.filePath && setting.filePath !== '') {
                    var type = setting.filePath.split('.').reverse()[0].toLocaleLowerCase();
                    $.each(setting.allowType, function(i, val) {
                        setting.allowType[i] = val.split('/')[1];
                    });
                    if ($.inArray(type, setting.allowType) > -1 || !setting.allowType.length) {
                        var loadBox;
                        if (Base.fileUpArray) {
                            if (Base.fileUpArray[setting.filePath]) {
                                callback(Base.fileUpArray[setting.filePath], setting.filePath, ele);
                                return;
                            }
                        }
                        $.ajaxFileUpload({
                            url: Base.web_root + '/sm_background/api/upload/image',
                            secureuri: false,
                            fileElementId: ele,
                            dataType: 'html',
                            beforeSend: function() {
                                loadBox = loadingBox($('body'));
                            },
                            complete: function() {
                                loadBox.close();
                            },
                            success: function(data, status) {
                                ...
                                if (caData.state === 200) {
                                    //缓存上传
                                    if (!Base.fileUpArray) {
                                        Base.fileUpArray = new Object();
                                    }
                                    Base.fileUpArray[setting.filePath] = caData;
                                    callback(caData, setting.filePath, ele);
                                } else {
                                    $.alert(caData.msg);
                                }
                            },
                        ...
                        $.alert('请上传以下文件类型：' + setting.allowType.join(','));
                    }
                }

            });
        },
        touchDrag: function(opts) {
            opts = $.extend({
                dragEnd: null
            }, opts);
            ...
            function Drag(dom) {
                ...
                var tStart = function(e) {
                    e.stoppropagation;
                    canDrag = true;
                    tx = dom.data('left') || parseInt(dom.css('left'));
                    ty = dom.data('top') || parseInt(dom.css('top'));
                    x = e.touches[0].pageX;
                    y = e.touches[0].pageY;
                };
                ...
                var tMove = function(e) {
                    if (canDrag && enabled){
                        ...
                        var s = ty + e.touches[0].pageY - y;
                        if(n < 0) n = 0;
                        if(n > wWidth - dWidth) n = wWidth - dWidth;
                        if(s < 0) s = 0;
                        if(s > wHeight - dHeight) s = wHeight - dHeight;
                        dom.css({
                            left: n,
                            top: s
                        }).data({
                ...
                var tEnd = function() {
                    canDrag = false;
                    if(opts.dragEnd) opts.dragEnd(dom);
                };

                dom.on('touchstart.dragTouch', tStart);
                dom.on('touchmove.dragTouch', tMove);
                dom.on('touchend.dragTouch', tEnd);
